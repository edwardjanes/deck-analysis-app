...
export async function GET(
  ...
  const { data, error } = await supabaseAdmin
    ...
    .select(
      "id, status, business_name, deck_file_path, created_at, score, verdict, verdict_type, most_damaging_issue, best_asset, analysis_summary, analysis_json, error_message, paid"
    )
    .eq("id", id)
    .single();
